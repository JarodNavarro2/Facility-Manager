...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		introduction();//Starting up the program
		Scanner user_intro = new Scanner(System.in);//User input
		String intro = user_intro.next();
		intro=intro.toLowerCase();//making it all lowercase
		boolean flag= false;//flag regarding exiting the program
		if (!intro.equals("exit"))//if the user doesn't type exit, run the program
		{
			flag = true;
		}
		while (flag == true)//While the flag stays true - user hasn't typed in "exit"
		{
			outerloop://Loop created to break the nested loops without breaking the entire loops
			if (intro.equals("1")) //WORKS - List Current Facilities
			{
				if (facility_list.isEmpty())//Can't list anything if nothing is in there
				{
			...
			else if (intro.equals("2")) //Add New Facilities - WORKS
			{
				...
				while (check_token) //while user has inputted a response beginning with "y" - assuming "yes"
				{
					...
					for (int j = 0; j < r_input; j++){
						...
						while (r_cc > r_mc){ //while user inputs a current capacity more than maximum capacity for a room, prompt results to try again...
						...
						if (fac.createARoom(r_name, r_cc, r_mc, r_num)) { //function returns true if room # is not taken and a room is documented to the facility list
							successful_room_creation = true;
						}
						else {
							while (!successful_room_creation) {
								System.out.println("Unable to create a room, because the desired room # is already assigned."); //only reason returns false
								System.out.print("Enter a desired room number: ");
								r_num = user_input.nextInt();
								if (fac.createARoom(r_name, r_cc, r_mc, r_num)) { //attempt #2
									successful_room_creation = true;
								}
							}
						}
			...
			else if (intro.equals("3")) ////See Facility Usage Rates
			{
				...
				if(input.equals("1")) //list inspections for a facility
				{
				...
				else if(input.equals("2")) //list actual usage for a facility
				{
					System.out.print("What is the name of the facility?");
					Scanner fac_in = new Scanner (System.in);
					String fac_out = fac_in.next();
				...
				else if(input.equals("3")) //calculate usage rates
				{
					System.out.print("What is the name of the facility?");
					Scanner fac_in = new Scanner (System.in);
					String fac_out = fac_in.next();
				...
				else if(input.equals("4")) //assign a facility to use
				{
					System.out.print("What is the name of the facility?");
					Scanner fac_in = new Scanner (System.in);
					String fac_out = fac_in.next();
			...
			{
				if (facility_list.isEmpty())///If the list is empty, you can't make requests
				{
				...
				else
				{
					...
					{
						//creating the maintenance request.
						...
						System.out.print("Enter the time (24 hour) for " + _add + ": ");
						...
						System.out.print("Enter the problem regarding this request");
						...
						request.calcProblemRateForFacility(req.getCurrentCapacity(), req.getMaxCapacity(), request);//Testing the problem rate
						if (request.problem_approval==true)//If the problem rate passes, tests the budget rate
						{
							System.out.println("Calculating funds");
							request.calcMaintenanceCostForFacility(request);//Testing budget cost
							if (request.money_approval == true)//If both pass, then add the request
							{							
								...
								for (Maintenance main : maintain_requests) 
								{
									...
									System.out.println("Request time: " + main.getSchedule_time()+ " hours");
									...
									request.setMaintenanceRequestBeginTime(); //Starting the downtime
								}
							}
						}
						else
						{
							System.out.println("Request denied");//If one of the calculations failed, it will not add
						}
						System.out.println("Add another request?");//Asking if the user wants to add more Maintenance requests
						ma = ma_input.next();
						if (ma.equals("no")) {
			...
			else if (intro.equals("5"))//Approving requests
			{
				if (maintain_requests.isEmpty())//Can't approve requests if there are no requests
				{
					System.out.println("No requests at this time");
					break outerloop;
				}
				Maintenance temp = new Maintenance();
				System.out.println("Listing requests and problems");
				int counter = 1;
				for (Maintenance main : maintain_requests) {
					System.out.println("Request #" + counter + " information.");
					System.out.println("Request name: " + main.getMaintenance_name());
					System.out.println("Request scheduled time: " + main.getSchedule_time()+ " hours");
					System.out.println("Request budget: " + main.getTotal_budget());
					System.out.println("Problem: "+ main.getProblem());
					System.out.println("Request cost: " + main.getTotal_cost());
					counter++;
				...
				if (!answer.isEmpty())//listing the requests
				{
				...
				System.out.println("Request scheduled time: "+ temp.getMaintenanceRequest(answer).getSchedule_time()+ " hours");
				...
				if (approve.startsWith("y"))//approving request.
				{
					...
					temp.getMaintenanceRequest(answer).setMaintenanceRequestCompletionTime();//Ending the timer. 
					temp.listFacilityProblems();//Listing the problems for that maintenance
					temp.calcDownTimeForFacility();//Calculating downtime
					break outerloop;
			...
			else if (intro.equals("10"))//Vacating the facilitiy
			{
				if (facility_list.isEmpty())
				{
			...
			else if (intro.equals("11")){
				if (facility_list.isEmpty())
				{
					System.out.println("No facilities exist, therefore unable to remove.");
					break outerloop;
				}
				Scanner delete = new Scanner (System.in);
				System.out.print("Enter the facility name for which you would like to delete: ");
				String delete_fac = delete.next();
				delete_fac=delete_fac.toLowerCase();
				int position = -1;
				for(int i = 0; i < facility_list.size(); i ++){
					if (facility_list.get(i).getFacilityName().toLowerCase().matches(delete_fac)){
						position = i;
					}
				}
				if (position != -1){
					facility_list.remove(position);
					System.out.print("Facility has been deleted.");
				}
				else {
	...
	public static void introduction()///introduction. Always reminding the user of their options
	{
		...
		System.out.println("11) Remove Facility");
		System.out.println("Type 'Exit' to leave the program"); //Working 02.05 - Jarod
	}

	public static Facility getFacility(String name, ArrayList<Facility> facList) ///Getting specific facilities 
	{
		Facility fac = null;
